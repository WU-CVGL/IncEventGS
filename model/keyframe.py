...
class KeyFrameDatabase(object):
    ...
    def sample_single_keyframe_rays(self, rays, option='random'):
        ...
        # sample rays according to index
        '''
        if option == 'random':
    ...
    def sample_global_rays(self, bs):
        ...
        '''
        num_kf = self.__len__()
        idxs = torch.tensor(random.sample(range(num_kf * self.num_rays_to_save), bs))  # random rays from all keyframe
        sample_rays = self.rays[:num_kf].reshape(-1, 7)[idxs]  # pre-saved rays(images)

        frame_ids = self.frame_ids[idxs//self.num_rays_to_save]  # rays from which frames
