...
def add_new_gs(
    params: Union[Dict[str, torch.nn.Parameter], torch.nn.ParameterDict],
    optimizers: Dict[str, torch.optim.Optimizer],
    new_gs: Dict[str, Tensor],
):
    """Inplace add new Gaussian points.

    Args:
        params: A dictionary of parameters (e.g., Gaussian means, covariances).
        optimizers: A dictionary of optimizers, each corresponding to a parameter.
        new_gs: A dictionary containing the new Gaussian points to add. Keys should match params.
    """
    device = new_gs["means"].device  # Get the device information from the new Gaussian points
    num_new_pts = new_gs["means"].shape[0]  # Get the number of new Gaussian points

    def param_fn(name: str, p: Tensor) -> Tensor:
        # Concatenate the existing parameters with the corresponding new Gaussian points
        return torch.nn.Parameter(torch.cat([p, new_gs[name]], dim=0))

    def optimizer_fn(key: str, v: Tensor) -> Tensor:
        # Initialize new optimizer state for the new points, e.g., zeros
        return torch.cat([v, torch.zeros((num_new_pts, *v.shape[1:]), device=device)], dim=0)
